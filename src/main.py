...
def register_user(user: UserRegistration, tasks: BackgroundTasks):
    ...
    admin_msg = f"New user has been registrated: {user.username} (chat_id={user.chat_id})"
    tasks.add_task(send_telegram_notification, admin_msg)

    user_msg = f"Hello, {user.username}! You are successfully register in the system. Your ID: {user_id}"
    tasks.add_task(send_telegram_notification, user_msg, [user.chat_id])
